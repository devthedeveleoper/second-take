      ...
      <div className="bg-surface border border-border rounded-xl p-6 shadow-sm flex flex-col gap-4">
        ...
        <div className="flex items-end justify-between h-32 gap-2 mt-auto">
          {ratingsDistribution.map((count, index) => {
            ...
            return (
              <div key={star} className="flex flex-col items-center gap-2 flex-1 group h-full">
                <span className="text-xs text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity">
                  {count}
                </span>
                <div className="w-full bg-surface-hover rounded-t-md relative flex items-end justify-center h-full">
                  <div 
                    className="w-full rounded-t-sm transition-all duration-700 opacity-80" 
                    style={{ height: `${heightPercent}%`, backgroundColor: 'var(--accent)' }} 
                  />
                </div>
      ...
      <div className="bg-surface border border-border rounded-xl p-6 shadow-sm flex flex-col gap-4">
        ...
        <div className="flex items-end justify-between h-32 gap-2 mt-auto">
          {[...monthlyActivity].reverse().map((data) => {
            ...
            return (
              <div key={data.month} className="flex flex-col items-center gap-2 flex-1 group h-full">
                <span className="text-xs text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity">
                  {data.count}
                </span>
                <div className="w-full bg-surface-hover rounded-t-md relative flex items-end justify-center h-full">
                  <div 
                    className="w-full rounded-t-sm transition-all duration-700 opacity-60" 
                    style={{ height: `${heightPercent}%`, backgroundColor: 'var(--accent)' }} 
                  />
                </div>
