...
export default function CastCarousel({ cast }: { cast: any[] }) {
  ...
  return (
    <div className="relative group">
      ...
      >
        {cast.map((actor: any) => (
          <div key={actor.id} className="flex flex-col gap-2 shrink-0 w-[120px] sm:w-[140px] snap-start">
            <div className="aspect-[2/3] relative rounded-md overflow-hidden bg-surface-hover shadow-sm border border-border group/actor">
              {actor.profile_path ? (
                <Image
                  ...
                  fill
                  sizes="150px"
                  className="object-cover group-hover/actor:scale-105 subtle-transition"
                />
              ) : (
            ...
            <div>
              <p className="font-medium text-sm leading-tight truncate">{actor.name}</p>
